...
def mary_tts_api_process():
    """MaryTTS-compatible /process endpoint with file upload support and MP3 output."""
    ...
    if file and file.filename.endswith('.txt'):
        ...
        print(f" > Model input: {text}")
        speaker_idx = request.headers.get("speaker-id") or request.values.get("speaker_id", "p376")
        wavs = synthesizer.tts(text, speaker_idx)

        wav_io = io.BytesIO()
        synthesizer.save_wav(wavs, wav_io)
        wav_io.seek(0)
